...
describe('一行几个题号', () => {
    ...
    test('老版本那个 256px 的卡是 5 列', () => {

        // 卡宽现在是屏幕的百分比（CONFIG.cardPercent），不再有一个固定的
        // 「默认卡宽」。这条留的是那个 256px 的锚点 —— 它对应的是卡宽的
        // **下限**，也就是屏幕窄到占比不敷用时卡实际拿到的宽度。
        //
        // 256px 的卡：减去面板 1px 边框两侧、body 24px 内边距两侧，题号区
        // 实宽 206px。内边距和卡宽是一直一起调的，目的是让留白变大而列数
        // 纹丝不动（11 → 16 → 24 配 220 → 240 → 256，实宽一直是 206），
        // 谁再单独动其中一个都会撞到这儿。
        assert.equal(
            layout.computeColumns(206, layout.CONFIG.minCellWidth, layout.CONFIG.gridGap),
            5,
            '256px 的卡排出来的列数变了，答题卡的样子跟着变，这是没打算发生的'
        );
    });

    test('卡宽的下限和内边距得配上 —— 那是它最窄的样子', () => {

        // 上面那条锚的是 206 那个宽度，但它现在只在下限那一档成立（宽屏
        // 上卡更宽、列数更多）。下限和内边距得真的推得出三列以上，否则
        // 宽屏再好看，屏幕一窄题号就挤成一条。
        //
        // 下面那条算式里的 24 必须和 .fbac-body 的内边距一致，断言盯着。
        assert.match(
            cssBlock('.fbac-body {'),
            /padding:\s*24px/,
            '.fbac-body 的内边距不是 24px —— 下面那条算式里的 24 就成了一句谎话'
        );
